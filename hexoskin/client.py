    ...
    def create(self, data, auth=None, *args, **kwargs):
        ...
        if response.result:
            return self.api._object_cache.set(
                ApiResourceInstance(response.result, self)
            )
        else:
    ...
    def _build_response(self, response):
        ...
        if ctype == "application/json":
            ...
            if is_data:
                ...
                if response.result.get("meta", {}).keys() > {
                    "limit",
                    "next",
                    "previous",
                }:
...
class ApiResourceList(ApiResultList):
    ...
    def _append_response(self, response, prepend=False):
        try:
            ...
            raise ApiError(
                "Cannot parse results, unexpected content received! %s \nFirst 64 chars of content: %s"
                % (e, response.body[:64])
            )
...
class ApiHelper(object):
    ...
    def _parse_base_url(self, base_url):
        ...
        raise ValueError(
            "Unable to determine URL from provided base_url arg: %s.", base_url
        )
    ...
    def _request(
        ...
        if (
            data
            and not isinstance(data, strtypes)
...
class HexoApi(ApiHelper):
    def __init__(
        ...
        return super(HexoApi, self).__init__(
